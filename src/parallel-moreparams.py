...
def esoclimi_emulate(Parameter_set,nSigmaCrit,nTlim,SigmaCritParams,TlimParams,nPressExceeded,nIntegrationError, PressExceededParams,IntegrationErrorParams):
    ...
    if exitValue< -101.:
        ...
        print 'Parameters: ', Parameter_set['ecc'], Parameter_set['obl'], Parameter_set['dist'], Parameter_set['p']
        comm.Free()
        exit(-200)
    elif np.abs(exitValue + 100) < 0.001 :
        nIntegrationError += 1
        IntegrationErrorParams[0] = np.append(IntegrationErrorParams[0],Parameter_set['p'])
        ...
        nPressExceeded += 1
        PressExceededParams[0] = np.append(PressExceededParams[0],Parameter_set['p'])
