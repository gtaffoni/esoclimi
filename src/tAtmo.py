...
def tAtmo(exop_par_file_name, workdir, logfile):
   ...
   from atmosphereGeometry import atmosphereGeometry,MixtureOneGasScaling
   from pipeline_interface import exop_par_file
   from atmosphereLib import air,CO2,H2O
   ...
   P_CO2_REF=380.
   MolarFraction_CO2_REF=P_CO2_REF*1e-6
   CO2_SCALING=MixtureOneGasScaling(air,CO2,P_CO2_REF)
   ...
   Surface_P_CO2=float(EXOP['P_CO2'][2])
   ...
   ps=Surface_DryPressure/(1-Surface_P_CO2*1e-6*0)
   Kin=100

   Surface_P_CO2=Surface_P_CO2*Kin
   AIR=CO2_SCALING(Surface_P_CO2)

   logfile.write("K=                       : %f\n"%(Surface_P_CO2/CO2_SCALING.P_REF))

   # model calculation
   tic=time.time()
   MA=PhysMoistAdiabat(condensible=H2O,noncon=AIR)
   MA.ptop=ptop
   ...
   Creation_code="tAtmo.py"
   Creation_code_version='1.0-2017 may 10-2017 Nov 16-'
...
#
#This version of tAtmo permorms scaling for P_CO2
#
