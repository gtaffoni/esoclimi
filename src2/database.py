...
global SigmaCritParams=[ np.empty(shape=0), np.empty(shape=0), np.empty(shape=0), np.empty(shape=0)]
global TlimParams= [ np.empty(shape=0), np.empty(shape=0), np.empty(shape=0), np.empty(shape=0)]

#parameters that are fixed in this run...
global gg         = 0 #geography
global fo_const   = 0.7 #ocean fraction (only for gg=0)
global p_CO2_P    = 38000 #CO2 partial pressure IN PPVM
global TOAalbfile = 'ALB_g5_rh60_co2x100.txt' #chech that these are consistent with p_CO2_P
global OLRfile    = 'OLR_g1_rh60_co2x100.txt'
...
def make_work_area (_dir):
    ...
    shutil.copy(template_dir+"/Planets/"+planet+".h", workDir+"planet.h")
    if planet == "EARTH":
